...
{	
	...
	if(argc == 1)
		data = read_problem(DEFAULT_DATA_FILE);
	else
		data = read_problem(argv[argc-1]);

	numunits[0] = data->numdims;
	
	weights = DBN_deepauto(data->data,data->numrows, maxepochpre, numlayers, numunits, batchsizepre, typesoflayers, &params);
